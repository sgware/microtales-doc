...
This document describes MicroTales, a single storytelling domain based on very simple fantasy Computer Role Playing Games. Each problem, which we call a \emph{scenario}, has:
\begin{itemize}
...
\noindent We define each of these parts in detail. MicroTales scenarios are highly modular, with different elements becoming available only when certain \hyperref[sec:extensions]{extensions} are enabled. For example, the \refmech{Undead} extension contains characters, items, locations, and actions related to ghosts. When the \refmech{Undead} extension is enabled, these elements can be used in a scenario.
...
\end{code}}
\vspace{-1em}
\caption{An example MicroTales scenario in YAML format.}
\label{fig:problemex}
\end{figure}
...
\begin{itemize}
	...
	\item An optional version number, to check compatibility. This is the version of MicroTales, not of the scenario itself.
	...
	\item A list of initial value assignments to the scenario's \hyperref[sec:variables]{variables}.
	\item A list of \hyperref[sec:goals]{goals} for the characters and the story itself.
\end{itemize}

\noindent Figure \ref{fig:problemex} shows an example scenario in YAML format.

Many elements of a scenario are not explicitly listed but are implied by what is explicitly listed. For example, each scenario defines \hyperref[sec:variables]{variables}, and the current state of the scenario is an assignment of a value to each of its variables. Variables are not explicitly listed because they are implied by the extensions, locations, and characters. Similarly, actions are not explicitly listed because which actions are available is implied by the other elements.

A scenario's elements cannot be created or destroyed. The only thing about a scenario that changes over time is the values assigned to its \hyperref[sec:variables]{variables}. New locations cannot be added to the map, nor can existing locations be removed. New characters cannot be created or removed (though they can be killed when the relevant \hyperref[sec:extensions]{extensions} are enabled). The items a character is carrying are not first class elements of a scenario, so they do not need to be explicitly defined, and they can be created and destroyed (\hyperref[sec:constants]{see the section on Constants for how items work}).

The following subsections define MicroTales elements in detail. Rules which must be followed are {\em required}. When something is {\em recommended}, it means that a scenario generator should try to follow those rules when it can, but it may choose to ignore those rules in service of some other design goal. When a definition says something is a {\em default}, it means that a scenario assumes the rule will be followed. If a default is not followed, the scenario must explicitly state some other value. For example, a \refchar{noble} character starts with a \refconst{Coin} in their \refvar{left} hand by default. The scenario does not need to list this under its initial facts because it will be assumed. If the noble should not start with a coin, the scenario should explicitly list their \refvar{left} hand as \None.

\paragraph{Style and Capitalization}
Throughout this document, we capitalize specific, individual objects. This includes universal constants like \element{True}, \element{False}, and \None, the names of all \hyperref[sec:extensions]{extensions}, the names of all \hyperref[sec:constants]{constants}, and the names given to individual objects defined in example scenarios, like the character name \element{Robin}. We use lower case for types, which includes all \hyperref[sec:locations]{location types}, \hyperref[sec:characters]{character types}, \hyperref[sec:variables]{variable names}, \hyperref[sec:goals]{goal names}, and \hyperref[sec:actions]{action names}. For example, \element{Robin} is the name of a specific character, so we capitalize his name, but his character type, \refchar{bandit}, is a type and is written in lower case. The variable \element{health(Robin)} has a lower case name because \refvar{health} is a type of variables, but the value \refconst{Healthy}, which can be assigned to that variable, is a single individual constant and is capitalized.
...
Figure \ref{fig:problemex} shows an example list of extensions. In YAML, the list of extensions is simply the key \element{extensions} followed by a sequence of strings. The list of extensions is optional and can be omitted, but scenarios with no extensions are likely to be uninteresting.

Some extensions require that other extensions are also enabled. For example, the \refmech{Undead} extension requires some way for characters to die, which is enabled by both the \refmech{Sickness} and \refmech{Violence} extensions. One of these two must be enabled in order to enable \refmech{Undead}. A scenario should explicitly list all extensions that it uses, even those required by other extensions. For example, a scenario that includes \refmech{Undead} should also explicitly list \refmech{Sickness} or \refmech{Violence} or both.

Several scenario elements can be enabled by more than one extension. For example, several extensions enable potions of some sort. All of these extensions enable the \refchar{sorcerer} character type, whose special actions revolve around using potions.
...
\begin{itemize}
	...
	\item Enables the \refgoal{rich} goal.
...
\begin{itemize}
	...
	\item Enables the \refgoal{lawful} and \refgoal{lawless} goals.
...
\begin{itemize}
  \item Enables the \refloc{chapel} location type.
	...
	\item Enables the \refconst{Flower} item constant.
	\item Enables the \refgoal{wed} goal type.
...
\begin{itemize}
  \item Enables the \refloc{castle} location type.
	...
	\item Enables the \refgoal{ruling} goal type.
...
\begin{itemize}
  \item Enables the \refchar{sorcerer} character type.
	...
	\item Enables the \refgoal{harmed} and \refgoal{unharmed} goal types.
...
\begin{itemize}
  \item Enables the \refchar{bandit} character type.
	...
	\item Enables the \refgoal{rich} goal type.
...
As shown in Figure \ref{fig:problemex}, in YAML the list of paths is the key \element{paths} followed by a sequence of pairs of strings that match the names of locations. The list of paths is optional if no locations are connected, though only very small scenarios will have no paths.
...
Figure \ref{fig:problemex} shows an example list of characters. In YAML, the list of characters is the key \element{characters} followed by a mapping of string names to string types, where the type is one of those defined below. The list of characters is not optional, because a scenario must define at least one character.
...
The only character type that is always available is the \refchar{peasant}, which has no special qualities. All other character types are enabled by \hyperref[sec:extensions]{extensions}.

\defchar{bandit}{
\begin{itemize}
	\item Only available when \refmech{Stealth} or \refmech{Theft} is enabled.
	\item Starts at the first \refloc{camp} or \refloc{crossroads} defined in the scenario by default.
	\item Required not to have \refgoal{lawful} goal.
	\item Recommended to have a \refgoal{rich} goal.
...
\begin{itemize}
	...
	\item Starts at the first \refloc{chapel} or \refloc{graveyard} defined in the scenario by default.
	\item Required not to have \refgoal{avenged} goal (unless they become a \refconst{Ghost}; see \hyperref[sec:goals]{Goals}).
	\item Recommended to have an \refgoal{obliged} goal.
...
\begin{itemize}
	...
	\item Starts at the first \refloc{castle} or \refloc{market} defined in the scenario by default.
	\item Starts with a \refconst{Sword} in their \refvar{left} hand by default.
	\item Recommended to have a \refgoal{ruling} goal for a \refchar{noble}.
	\item Recommended to have a \refgoal{lawful} goal.
...
\begin{itemize}
	\item Only available when \refmech{Commerce} is enabled.
	\item Starts at the first \refloc{market} or \refloc{crossroads} defined in the scenario by default.
	\item Recommended to start with at least one item of any type except a \refconst{Coin}.
	\item Required not to have a \refgoal{liked} goal.
	\item Recommended to have a \refgoal{rich} goal.
...
\begin{itemize}
	...
	\item Starts at the first \refloc{castle} or \refloc{chapel} defined in the scenario by default.
	\item Starts with a \refconst{Coin} in their \refvar{left} hand by default.
	\item Recommended to have a \refgoal{ruling} goal.
	\item Can \refact{charm} another character even when not carrying a \refconst{CharmPotion}.
\end{itemize}}

\defchar{peasant}{
\begin{itemize}
	\item Starts at the first \refloc{village} or \refloc{workshop} defined in the scenario by default.
\end{itemize}}
...
\begin{itemize}
	...
	\item Starts at the first \refloc{laboratory} or \refloc{graveyard} defined in the scenario by default.
	\item Recommended to start with at least one \hyperref[sec:constants]{potion} of any type.
	\item Recommended to have an \refgoal{avenged} goal.
...
\begin{itemize}
  \item Only available when \refmech{Crime} is enabled.
	...
	\item The \refgoal{lawless} goal is satisfied when characters are criminals and not in a \refloc{jail}.
	...
	\item It is a crime to \refact{release} criminals from the \refloc{jail}.
...
\begin{itemize}
  \item Only available when \refmech{Sickness} or \refmech{Violence} is enabled.
	...
	\item The \refgoal{harmed} goal is satisfied by a character not being \refconst{Healthy}.
	\item The \refgoal{unharmed} goal is satisfied by a character being \refconst{Healthy}.
...
\begin{itemize}
  \item The \param{character}'s current position on the map.
	...
	\item \hyperref[sec:characters]{Character types} specify default locations. When the specified locations are not available, the default value is the first location defined in the scenario.
	\item The \refgoal{at} goal is satisfied by a character being at a certain location.
...
\begin{itemize}
  \item Only available when \refmech{Monarchy} is enabled.
	...
	\item The \refgoal{ruling} goal is satisfied by a certain character becoming the monarch.
...
\begin{itemize}
  \item How the first \param{character} views the \param{other} character. The arguments must be different characters.
	...
	\item The \refgoal{avenged} goal is satisfied by a character not regarding other living characters as \refconst{Enemy}.
	\item The \refgoal{disliked} goal is satisfied by other characters regarding a character as an \refconst{Enemy}.
	\item The \refgoal{liked} goal is satisfied by other characters regarding a character as a \refconst{Friend}.
	\item The \refgoal{obliged} goal is satisfied by a character regarding other \refconst{Healthy} characters as \refconst{Friend}.
...
\begin{itemize}
  \item Only available when \refmech{Marriage} is enabled.
	...
	\item The \refgoal{wed} goal is satisfied when a pair of characters are one another's spouses.
...
Constants are special values that can be assigned to some \hyperref[sec:variables]{variables}. Constants are not explicitly defined; they are implied by which \hyperref[sec:extensions]{extensions} are enabled. For example, the \refconst{Ghost} constant is one possible value for a \refvar{health} variable, and it represents a character who has been raised from the dead. However, if the \refmech{Undead} extension is not enabled, this constant is not available, even if the other constants that can be assigned to \refvar{health} variables are.
...
\begin{itemize}
  \item Only available when \refmech{Commerce} or \refmech{Theft} is enabled.
	...
	\item The \refgoal{rich} goal is satisfied by having coins.
	\item Any item can be converted into a coin via the \refact{sell} action at the \refloc{market}.
	\item When \refmech{Crafting} is enabled, it can be \refact{craft}ed from any other metal item or into any other metal item.
\end{itemize}}
...
\begin{itemize}
  \item Assigned to a \refvar{relationship} to indicate the first character regards the second negatively.
	\item The \refgoal{disliked} goal is satisfied by other characters regarding a character this way.
	\item The \refgoal{avenged} goal is satisfied by a character not regarding others this way, or by a character's enemies being dead.
...
\begin{itemize}
  \item Assigned to a \refvar{relationship} to indicate the first character regards the second positively.
	\item The \refgoal{obliged} goal is satisfied by a character regarding others this way, or by a character's friends becoming healthy.
	\item The \refgoal{liked} goal is satisfied by other characters regarding a character this way.
...
Similar to \hyperref[sec:variables]{variables}, a goal is one of the names define below followed by zero to many arguments. Figure \ref{fig:problemex} shows an example list of goals. In YAML, it is the key \element{goals} followed by a sequence of sequences. If the sequence is a character goal, the first element is the name of one of the scenario's \hyperref[sec:characters]{characters}. If the sequence is a story goal, this name is omitted. The next elements in both types are the goal name (defined below), followed by the goal's arguments.

For example, this character goal in YAML format says that \element{John} wants \element{John} to be the \refvar{monarch}:

\centerline{\element{[John, ruling, John]}}

\noindent The first part, \element{John}, specifies which character has this goal. The rest specifies the goal \element{ruling(John)}, which is the objective that \element{John} be the \refvar{monarch}. Characters can also have goals for other characters:

\centerline{\element{[Sheriff, ruling, John]}}

\noindent The goal of \element{ruling(John)} is the same, but it is held by the \element{Sheriff}. The \element{Sheriff} also wants \element{John} to be \refvar{monarch}.

To make this a story goal instead of a character goal, we simply omit the character from the start of the sequence:

\centerline{\element{[ruling, John]}}

\noindent This means that \element{ruling(John)} is a goal for the story as a whole rather than any particular character. The storyteller should try to tell a story where \element{John} becomes the \refvar{monarch}.

Two character goals always apply and do not need to be explicitly listed. When \refmech{Sickness} or \refmech{Violence} is enabled, all characters prefer being \refconst{Healthy} over \refconst{Hurt} and prefer \refconst{Hurt} over \refconst{Dead}. Second, if \refmech{Undead} is enabled, all \refconst{Ghost}s have the \refgoal{avenged} goal toward themselves. This means they want to kill any character they regard as an \refconst{Enemy}.

Goals are persistent, meaning they do not go away once they are achieved. For example, when a character has the goal to be \refgoal{rich}, they want \refconst{Coin}s. Once both of their hands are holding \refconst{Coin}s, they cannot have more, but they do not stop having the goal to be \refgoal{rich}. If they should ever lose one of their coins, they will again want more.

MicroTales does not define the relative importance of one goal to another, and when a goal can be achieved in multiple ways, it only defines ordinal preferences. For example, if a character wants to be both \refgoal{rich} and \refgoal{obliged}, MicroTales does not specify whether it is more important to that character to have \refconst{Coin}s or \refconst{Friend}s; this is up to the storyteller. Similarly, a character who wants to be \refgoal{rich} would prefer to have 2 \refconst{Coin}s over 1 \refconst{Coin}, but it is not necessarily twice as good to have 2 than it is to have 1. Again, this is left up to the storyteller.
...
\defgoal{at}{\param{character}, \param{location}}{
\begin{itemize}
	\item Achieved when \param{character}'s \refvar{location} is \param{location}.
\end{itemize}}

\defgoal{avenged}{\param{character}}{
	\begin{itemize}
		...
		\item Opposed to \refgoal{obliged}.
...
\defgoal{disliked}{\param{character}}{
	\begin{itemize}
...
\begin{itemize}
	...
	\item Opposed to \refgoal{lawless}.
...
\begin{itemize}
	...
	\item Opposed to \refgoal{disliked}.
...
\defgoal{obliged}{\param{character}}{
\begin{itemize}
	...
	\item Opposed to \refgoal{avenged}.
...
\defgoal{unharmed}{\param{character}}{
\begin{itemize}
  \item Only available when \refmech{Sickness} or \refmech{Violence} is enabled.
	\item Achieved when \param{character}'s \refvar{health} is \refconst{Healthy}.
	\item Opposed to \refgoal{harmed}.
\end{itemize}}

\defgoal{wed}{\param{character}, \param{spouse}}{
\begin{itemize}
	\item Only available when \refmech{Marriage} is enabled.
...
A scenario does not explicitly define actions; they are implied by the other elements in the scenario. Any time an action is enabled by a \hyperref[sec:extensions]{extension} it is available in a story. For example, the \refact{enthrone} action is always available in every story where \refmech{Monarchy} is enabled, though it can only happen when a character is in a \refloc{castle} and holding a \refconst{Crown}.
...
The only actions that always available in every scenario are basic movement, the \refact{walk} action, and basic item interactions, the \refact{pickup}, \refact{drop}, \refact{give}, and \refact{trade} actions.
...
\begin{eff}
  \item If the \param{target} was \refconst{Healthy}, they are now \refconst{Hurt}.
	...
	\item If the \refvar{relationship} between \param{target} and \param{character} was \refconst{Friend}, it is now \None.
	\item If the \refvar{relationship} between \param{target} and \param{character} was \None, it is now \refconst{Enemy}.
	\item If the \param{target} was \refconst{Hurt} (now \refconst{Dead}) and \param{target}'s \refvar{spouse} was \param{other}, then the \param{target}'s spouse is now \None and \param{other}'s \refvar{spouse} is now \None.
\end{eff}}
...
\begin{pre}
  \item Only available when \refmech{Sickness} or \refmech{Undead} is enabled.
	\item The \param{character} and \param{target} character may be the same.
	\item The \param{character} is alive and holding a \refconst{CursePotion}, or the \param{character} is a \refconst{Ghost}.
...
\begin{eff}
  \item If the \param{target} was \refconst{Healthy}, they are now \refconst{Hurt}.
	\item If the \param{target} was \refconst{Hurt}, they are now \refconst{Dead}.
	\item If the \param{character} is not a \refconst{Ghost}, they are now holding one fewer \refconst{CursePotion}.
	\item If the \param{target} is not a \refvar{criminal}, the \param{character} is now a \refvar{criminal}.
	\item If the \refvar{relationship} between \param{target} and \param{character} was \refconst{Friend}, it is now \None.
	\item If the \refvar{relationship} between \param{target} and \param{character} was \None, it is now \refconst{Enemy}.
	\item If the \param{target} was \refconst{Hurt} (now \refconst{Dead}) and \param{target}'s \refvar{spouse} was \param{other}, then the \param{target}'s spouse is now \None and \param{other}'s \refvar{spouse} is now \None.
...
\begin{pre}
  \item Only available when \refmech{Stealth} is enabled.
	\item The \param{character} is alive.
	\item The \param{character} is \refvar{visible}.
	\item The \param{character} is at a \refloc{camp}, is a \refchar{bandit}, or is holding a \refconst{HidePotion}.
\end{pre}
\begin{eff}
  \item The \param{character} is no longer \refvar{visible}.
  \item If the \param{character} is not at a \refloc{camp} and is not a \refchar{bandit}, the \param{character} now has one fewer \refconst{HidePotion}.
\end{eff}}
