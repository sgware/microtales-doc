...
\noindent We define each of these parts in detail. MicroTales problems are highly modular, with different elements becoming available only when certain \hyperref[sec:extensions]{extensions} are enabled. For example, the \refmech{Undead} extension contains characters, items, locations, and actions related to ghosts. When the \refmech{Undead} extension is enabled, these elements can be used in a problem.
...
Figure \ref{fig:problemex} shows an example list of extensions. In YAML, the list of extensions is simply the key \element{extensions} followed by a sequence of strings. The list of extensions is optional and can be omitted, but problems with no extensions are likely to be uninteresting.

Some extensions require that other extensions are also enabled. For example, the \refmech{Undead} extension requires some way for characters to die, which is enabled by both the \refmech{Sickness} and \refmech{Violence} extensions. One of these two must be enabled in order to enable \refmech{Undead}. A problem should explicitly list all extensions that it uses, even those required by other extensions. For example, a problem that includes \refmech{Undead} should also explicitly list \refmech{Sickness} or \refmech{Violence} or both.

Several problem elements can be enabled by more than one extension. For example, several extensions enable potions of some sort. All of these extensions enable the \refchar{sorcerer} character type, whose special actions revolve around using potions.
...
\begin{itemize}
	...
	\item Starts at the first \refloc{camp} or \refloc{crossroads} defined in the problem by default.
...
A problem does not explicitly define its variables; they are implied by the other elements in the problem. For example, when the \refmech{Violence} extension is enabled the problem must have a \refvar{health} variable for every \hyperref[sec:characters]{character}.
...
A problem does not explicitly define actions; they are implied by the other elements in the problem. Any time an action is enabled by a \hyperref[sec:extensions]{extension} it is available in a story. For example, the \refact{enthrone} action is always available in every story where \refmech{Monarchy} is enabled, though it can only happen when a character is in a \refloc{castle} and holding a \refconst{Crown}.
